...
# Inluir clases al grafo (atributo RDFS.Class) por orden en la slide
...
# Inluir subclases al grafo (atributo RDFS.subClassOf y se indica de que clases son)
...
# Inluir etiquetas al grafo (todas son string)
...
# hasName: cada persona tiene un nombre (debe ser literal string ya que no tiene rango)
...
# hasHomePage: cada profesor tiene su pagina (debe ser literal string ya que no tiene rango)
...
# hasColleague: cada persona tiene un colega (en este caso el rango debe ser otra persona)
...
# Asun (full professor), tiene pagina y colega raul
...
# Oscar (associate professor) tiene nombre y colega asun 
...
# Raul (interin professor)no tiene relaciones
