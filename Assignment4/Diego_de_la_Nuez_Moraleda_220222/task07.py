...
for cls in g.subjects(RDF.type, RDFS.Class):
    # Obtenemos la lista de superclases de cada clase
...
for s, p, o in g.triples((None, RDF.type, None)):
    # Si el tipo es Person o una subclase de Person
    clase = o
    while clase is not None:
        if clase == ns.Person:
            individuals.append(s)
            break
        # Buscar si tiene superclase
        superclasses = list(g.objects(clase, RDFS.subClassOf))
        clase = superclasses[0] if superclasses else None
...
WHERE {
  ...
  ?colleague ontology:ownsPet ?petName .
  ?person rdfs:label ?name .
}
"""
