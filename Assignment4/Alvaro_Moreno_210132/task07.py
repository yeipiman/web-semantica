...
Original file is located at
    https://colab.research.google.com/drive/1Pgdu7JekRnOzjEJgLSfawUjhDJOO-byM
...
for c in classes:
    ...
    superclasses = [sc for sc in superclasses if sc != c]
...
all_person_classes = {target_class}
classes_to_explore = [target_class]

while classes_to_explore:
    current_class = classes_to_explore.pop()

    for subclass in g.subjects(RDFS.subClassOf, current_class):
        if subclass not in all_person_classes and subclass != RDFS.Resource:
            all_person_classes.add(subclass)
            classes_to_explore.append(subclass)
...
WHERE {
    ?x (ns:hasColleague | ns:hasColleague/ns:hasColleague) ?colleague_with_pet .
    ?colleague_with_pet ns:ownsPet ?pet .
